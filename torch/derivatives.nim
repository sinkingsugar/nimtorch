...
autograd masked_fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; mask: Tensor; value: float): Tensor = 
    ty.atenMethod("masked_fill_", self.tensor, mask.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().masked_fill_inplace(mask, 0))

autograd masked_fill_inplace:
  proc forward*(self: Tensor; mask: Tensor; value: float): Tensor = 
    self.tensor.atenMethod("masked_fill_", mask.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().masked_fill_inplace(mask, 0))

autograd masked_fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; mask: Tensor; value: Tensor): Tensor = 
    ty.atenMethod("masked_fill_", self.tensor, mask.tensor, value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().masked_fill_inplace(mask, 0))
  value: firstOrSelf(where(mask, grad, zeros_like(grad)).sum())

autograd masked_fill_inplace:
  proc forward*(self: Tensor; mask: Tensor; value: Tensor): Tensor = 
    self.tensor.atenMethod("masked_fill_", mask.tensor, value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().masked_fill_inplace(mask, 0))
  value: firstOrSelf(where(mask, grad, zeros_like(grad)).sum())
...
autograd put_inplace:
  proc forward*(ty: TensorType; self: Tensor; index: Tensor; source: Tensor; accumulate: bool = false): Tensor = 
    ty.atenMethod("put_", self.tensor, index.tensor, source.tensor, accumulate).to(ATensor).newTensor()
...
autograd put_inplace:
  proc forward*(self: Tensor; index: Tensor; source: Tensor; accumulate: bool = false): Tensor = 
    self.tensor.atenMethod("put_", index.tensor, source.tensor, accumulate).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().put_inplace(index, zeros_like(source), accumulate))
  source: firstOrSelf(grad.take(index))

autograd index_add_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; source: Tensor): Tensor = 
    ty.atenMethod("index_add_", self.tensor, dim, index.tensor, source.tensor).to(ATensor).newTensor()
...
autograd index_add_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; source: Tensor): Tensor = 
    self.tensor.atenMethod("index_add_", dim, index.tensor, source.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad)
  source: firstOrSelf(grad.index_select(dim, index))

autograd index_fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; value: float): Tensor = 
    ty.atenMethod("index_fill_", self.tensor, dim, index.tensor, value).to(ATensor).newTensor()
...
autograd index_fill_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; value: float): Tensor = 
    self.tensor.atenMethod("index_fill_", dim, index.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().index_fill_inplace(dim, index, 0))

autograd index_fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; value: Tensor): Tensor = 
    ty.atenMethod("index_fill_", self.tensor, dim, index.tensor, value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().index_fill_inplace(dim, index, 0))
  value: firstOrSelf(grad.index_select(dim, index).sum())

autograd index_fill_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; value: Tensor): Tensor = 
    self.tensor.atenMethod("index_fill_", dim, index.tensor, value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().index_fill_inplace(dim, index, 0))
  value: firstOrSelf(grad.index_select(dim, index).sum())

autograd scatter_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; src: Tensor): Tensor = 
    ty.atenMethod("scatter_", self.tensor, dim, index.tensor, src.tensor).to(ATensor).newTensor()
...
autograd scatter_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; src: Tensor): Tensor = 
    self.tensor.atenMethod("scatter_", dim, index.tensor, src.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().scatter_inplace(dim, index, 0))
  src: firstOrSelf(grad.gather(dim, index))

autograd scatter_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; value: float): Tensor = 
    ty.atenMethod("scatter_", self.tensor, dim, index.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().scatter_inplace(dim, index, 0))

autograd scatter_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; value: float): Tensor = 
    self.tensor.atenMethod("scatter_", dim, index.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().scatter_inplace(dim, index, 0))

autograd scatter_add_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; src: Tensor): Tensor = 
    ty.atenMethod("scatter_add_", self.tensor, dim, index.tensor, src.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad)
  src: firstOrSelf(grad.gather(dim, index))

autograd scatter_add_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; src: Tensor): Tensor = 
    self.tensor.atenMethod("scatter_add_", dim, index.tensor, src.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad)
  src: firstOrSelf(grad.gather(dim, index))

autograd lt_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("lt_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd lt_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("lt_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd lt_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("lt_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd lt_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("lt_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd gt_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("gt_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd gt_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("gt_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd gt_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("gt_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd gt_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("gt_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd le_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("le_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd le_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("le_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd le_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("le_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd le_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("le_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd ge_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("ge_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd ge_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("ge_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd ge_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("ge_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd ge_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("ge_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd eq_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("eq_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd eq_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("eq_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd eq_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("eq_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd eq_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("eq_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd ne_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: float): Tensor = 
    ty.atenMethod("ne_", self.tensor, other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd ne_inplace:
  proc forward*(self: Tensor; other: float): Tensor = 
    self.tensor.atenMethod("ne_", other).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))

autograd ne_inplace:
  proc forward*(ty: TensorType; self: Tensor; other: Tensor): Tensor = 
    ty.atenMethod("ne_", self.tensor, other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))

autograd ne_inplace:
  proc forward*(self: Tensor; other: Tensor): Tensor = 
    self.tensor.atenMethod("ne_", other.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(self))
  other: firstOrSelf(zeros_like(other))
...
autograd random_inplace:
  proc forward*(ty: TensorType; self: Tensor; from_name: int64; to_name: int64; generator: Generator = nil): Tensor = 
    ty.atenMethod("random_", self.tensor, from_name, to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd random_inplace:
  proc forward*(self: Tensor; from_name: int64; to_name: int64; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("random_", from_name, to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd random_inplace:
  proc forward*(ty: TensorType; self: Tensor; to_name: int64; generator: Generator = nil): Tensor = 
    ty.atenMethod("random_", self.tensor, to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd random_inplace:
  proc forward*(self: Tensor; to_name: int64; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("random_", to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd random_inplace:
  proc forward*(ty: TensorType; self: Tensor; generator: Generator = nil): Tensor = 
    ty.atenMethod("random_", self.tensor, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd random_inplace:
  proc forward*(self: Tensor; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("random_", generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
...
autograd uniform_inplace:
  proc forward*(ty: TensorType; self: Tensor; from_name: float64 = 0; to_name: float64 = 1; generator: Generator = nil): Tensor = 
    ty.atenMethod("uniform_", self.tensor, from_name, to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd uniform_inplace:
  proc forward*(self: Tensor; from_name: float64 = 0; to_name: float64 = 1; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("uniform_", from_name, to_name, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
...
autograd normal_inplace:
  proc forward*(ty: TensorType; self: Tensor; mean: float64 = 0; std: float64 = 1; generator: Generator = nil): Tensor = 
    ty.atenMethod("normal_", self.tensor, mean, std, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd normal_inplace:
  proc forward*(self: Tensor; mean: float64 = 0; std: float64 = 1; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("normal_", mean, std, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
...
autograd cauchy_inplace:
  proc forward*(ty: TensorType; self: Tensor; median: float64 = 0; sigma: float64 = 1; generator: Generator = nil): Tensor = 
    ty.atenMethod("cauchy_", self.tensor, median, sigma, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd cauchy_inplace:
  proc forward*(self: Tensor; median: float64 = 0; sigma: float64 = 1; generator: Generator = nil): Tensor = 
    self.tensor.atenMethod("cauchy_", median, sigma, generator).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
...
autograd hardtanh_inplace:
  proc forward*(ty: TensorType; self: Tensor; min_val: float; max_val: float): Tensor = 
    ty.atenMethod("hardtanh_forward_", self.tensor, min_val, max_val).to(ATensor).newTensor()
  self: firstOrSelf(hardtanh_backward(grad, fwd_result, min_val, max_val))

autograd hardtanh_inplace:
  proc forward*(self: Tensor; min_val: float; max_val: float): Tensor = 
    atenFunction("at::hardtanh_forward_", self.tensor, min_val, max_val).to(ATensor).newTensor()
  self: firstOrSelf(hardtanh_backward(grad, fwd_result, min_val, max_val))
...
autograd leaky_relu_inplace:
  proc forward*(ty: TensorType; self: Tensor; negative_slope: float): Tensor = 
    ty.atenMethod("leaky_relu_forward_", self.tensor, negative_slope).to(ATensor).newTensor()
  self: firstOrSelf(leaky_relu_backward(grad, fwd_result, negative_slope))

autograd leaky_relu_inplace:
  proc forward*(self: Tensor; negative_slope: float): Tensor = 
    atenFunction("at::leaky_relu_forward_", self.tensor, negative_slope).to(ATensor).newTensor()
  self: firstOrSelf(leaky_relu_backward(grad, fwd_result, negative_slope))
...
autograd rrelu_with_noise_inplace:
  proc forward*(ty: TensorType; self: Tensor; noise: Tensor; lower: float; upper: float; training: bool; generator: Generator): Tensor = 
    ty.atenMethod("rrelu_with_noise_forward_", self.tensor, noise.tensor, lower, upper, training, generator).to(ATensor).newTensor()
  self: firstOrSelf(rrelu_with_noise_backward(grad, fwd_result, noise, lower, upper, training))

autograd rrelu_with_noise_inplace:
  proc forward*(self: Tensor; noise: Tensor; lower: float; upper: float; training: bool; generator: Generator): Tensor = 
    atenFunction("at::rrelu_with_noise_forward_", self.tensor, noise.tensor, lower, upper, training, generator).to(ATensor).newTensor()
  self: firstOrSelf(rrelu_with_noise_backward(grad, fwd_result, noise, lower, upper, training))
...
autograd threshold_inplace:
  proc forward*(ty: TensorType; self: Tensor; threshold: float; value: float): Tensor = 
    ty.atenMethod("threshold_forward_", self.tensor, threshold, value).to(ATensor).newTensor()
  self: firstOrSelf(threshold_backward(grad, fwd_result, threshold, value))

autograd threshold_inplace:
  proc forward*(self: Tensor; threshold: float; value: float): Tensor = 
    atenFunction("at::threshold_forward_", self.tensor, threshold, value).to(ATensor).newTensor()
  self: firstOrSelf(threshold_backward(grad, fwd_result, threshold, value))
...
autograd fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; value: float): Tensor = 
    ty.atenMethod("fill_", self.tensor, value).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd fill_inplace:
  proc forward*(self: Tensor; value: float): Tensor = 
    self.tensor.atenMethod("fill_", value).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd fill_inplace:
  proc forward*(ty: TensorType; self: Tensor; value: Tensor): Tensor = 
    ty.atenMethod("fill_", self.tensor, value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
  value: firstOrSelf(grad.sum())

autograd fill_inplace:
  proc forward*(self: Tensor; value: Tensor): Tensor = 
    self.tensor.atenMethod("fill_", value.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
  value: firstOrSelf(grad.sum())
...
autograd index_copy_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64; index: Tensor; source: Tensor): Tensor = 
    ty.atenMethod("index_copy_", self.tensor, dim, index.tensor, source.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().index_fill_inplace(dim, index, 0))
  source: firstOrSelf(grad.index_select(dim, index))

autograd index_copy_inplace:
  proc forward*(self: Tensor; dim: int64; index: Tensor; source: Tensor): Tensor = 
    self.tensor.atenMethod("index_copy_", dim, index.tensor, source.tensor).to(ATensor).newTensor()
  self: firstOrSelf(grad.clone().index_fill_inplace(dim, index, 0))
  source: firstOrSelf(grad.index_select(dim, index))
...
autograd squeeze_inplace:
  proc forward*(ty: TensorType; self: Tensor): Tensor = 
    ty.atenMethod("squeeze_", self.tensor).to(ATensor).newTensor()
  self: firstOrSelf(unsqueeze_to(grad, self.sizes()))

autograd squeeze_inplace:
  proc forward*(self: Tensor): Tensor = 
    self.tensor.atenMethod("squeeze_").to(ATensor).newTensor()
  self: firstOrSelf(unsqueeze_to(grad, self.sizes()))

autograd squeeze_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64): Tensor = 
    ty.atenMethod("squeeze_", self.tensor, dim).to(ATensor).newTensor()
  self: firstOrSelf(unsqueeze_to(grad, dim, self.sizes()))

autograd squeeze_inplace:
  proc forward*(self: Tensor; dim: int64): Tensor = 
    self.tensor.atenMethod("squeeze_", dim).to(ATensor).newTensor()
  self: firstOrSelf(unsqueeze_to(grad, dim, self.sizes()))
...
autograd transpose_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim0: int64; dim1: int64): Tensor = 
    ty.atenMethod("transpose_", self.tensor, dim0, dim1).to(ATensor).newTensor()
  self: firstOrSelf(grad.transpose(dim0, dim1))

autograd transpose_inplace:
  proc forward*(self: Tensor; dim0: int64; dim1: int64): Tensor = 
    self.tensor.atenMethod("transpose_", dim0, dim1).to(ATensor).newTensor()
  self: firstOrSelf(grad.transpose(dim0, dim1))
...
autograd unsqueeze_inplace:
  proc forward*(ty: TensorType; self: Tensor; dim: int64): Tensor = 
    ty.atenMethod("unsqueeze_", self.tensor, dim).to(ATensor).newTensor()
  self: firstOrSelf(grad.squeeze(dim))

autograd unsqueeze_inplace:
  proc forward*(self: Tensor; dim: int64): Tensor = 
    self.tensor.atenMethod("unsqueeze_", dim).to(ATensor).newTensor()
  self: firstOrSelf(grad.squeeze(dim))
...
autograd zero_inplace:
  proc forward*(ty: TensorType; self: Tensor): Tensor = 
    ty.atenMethod("zero_", self.tensor).to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))

autograd zero_inplace:
  proc forward*(self: Tensor): Tensor = 
    self.tensor.atenMethod("zero_").to(ATensor).newTensor()
  self: firstOrSelf(zeros_like(grad))
